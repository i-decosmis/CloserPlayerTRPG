...
void loop() {
  if (Serial.available() > 0) {
    ...
    if (command == '1') {
      Serial.println("Motorino acceso");
      digitalWrite(in1, HIGH);  // Gira in senso orario
      digitalWrite(in2, LOW);
    } else if (command == '0') {
      Serial.println("Motorino spento");
      digitalWrite(in1, LOW);  // Spegne il motore
      digitalWrite(in2, LOW);
    } else if (command == 'L') {
      Serial.println("Motorino gira a sinistra");
      digitalWrite(in1, LOW);  // Gira in senso antiorario
      digitalWrite(in2, HIGH);
    } else if (command == 'R') {
      Serial.println("Motorino gira a destra");
      digitalWrite(in1, HIGH);  // Gira in senso orario
      digitalWrite(in2, LOW);
      ...
      Serial.print("Velocità cambiata a: ");
